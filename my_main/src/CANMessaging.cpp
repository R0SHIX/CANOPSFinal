    ...
    public:
        ...
        private:
            void detect_callback(const std_msgs::msg::Bool & msg){
                ...
                sendData = {0x22,0x17,0x45,0x00,0x00,0x00,0x00,0x00};
                SendMessage(ID, sendData, LEN);
                //if true
                //RCLCPP_INFO(this->get_logger(), "Message type: '%s'", msg.data);
                if(msg.data == true){
                    ...
                    std::this_thread::sleep_for(std::chrono::seconds(0.5));
                    */
                    sendData = {0x22,0x34,0x45,0x00,0x01,0x00,0x00,0x00};
                    SendMessage(ID, sendData, LEN);
		    RCLCPP_INFO(this->get_logger(), "Stop speed Enabled");

                }
	/*	else{
                    //A5 = 165 = 16.5
                    sendData = {0x22,0x34,0x45,0x00,0x00,0x00,0x00,0x00};
                    SendMessage(ID, sendData, LEN);
                     RCLCPP_INFO(this->get_logger(), "Disable");
